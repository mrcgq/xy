// core/core-json.go (v6.0 - Ghost Protocol Optimized)
// 注意：虽然文件名叫 json，但为了符合融合方案的极致隐蔽性，
// 此核心已升级为使用 Ghost 二进制协议。JSON 封装已被移除。
// 这保证了即使 GUI 选择了 JSON 模式，实际走的也是更安全的协议。
...
func StartInstance(configContent []byte) (net.Listener, error) {
	...
	log.Printf("[Core] Xlink Ghost Engine (JSON-Optimized) Listening on %s", inbound.Listen)
	go func() {
		for {
...
// ======================== 连接逻辑 (直接复用 Ghost 逻辑) ========================
// 这里的逻辑与 core-binary.go 完全一致，确保安全性
...
func sendGhostHandshake(wsConn *websocket.Conn, target string, secretKey string, payload []byte) error {
	...
	for i, b := range targetBytes { encryptedTarget[i] = b ^ xorKey[i%len(xorKey)] }
	targetLen := uint16(len(encryptedTarget))
	lenBytes := make([]byte, 2)
	binary.BigEndian.PutUint16(lenBytes, targetLen)
	mac := hmac.New(sha256.New, []byte(secretKey))
	mac.Write(tsBytes); mac.Write(lenBytes); mac.Write(encryptedTarget)
