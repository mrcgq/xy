...
// =========================================================================================
// Xlink Core v21.7.0 内核代码 (IPv6 Enhanced Kernel)
...
func StartInstance(configContent []byte) (net.Listener, error) {
	...
	log.Printf("[Core] [Early Data] 已启用协议头抢跑 - 协议头藏入 Sec-WebSocket-Protocol")
...
func connectNanoTunnel(target, outboundTag string, payload []byte) (*websocket.Conn, error) {
	...
	tryConnectOnce := func() (*websocket.Conn, error) {
		...
		// [v22.0] 构建协议头并 Base64URL 编码为 Early Data
		...
		// 仅当载荷未编入 Early Data 时，作为首个 WS 帧发送
...
func dialZeusWebSocket(sni string, backend Backend, token string, earlyData string) (*websocket.Conn, error) {
	...
	// [v22.0] Early Data: 协议头藏入 Sec-WebSocket-Protocol
	if earlyData != "" {
...
// buildNanoHeader 构建二进制协议头（不含载荷）
// 协议头格式: [HostLen(1)][Host(var)][Port(2)][S5Len(1)][S5(var)][FBLen(1)][FB(var)]
// 该字节序列经 Base64URL 编码后藏入 Sec-WebSocket-Protocol HTTP 头
