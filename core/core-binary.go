// core/core-binary.go (v20.3.1 - Intelligent Hybrid Edition)
// [核心升级] 引入“视频免疫 + 主动断流”混合智能策略
// [增强] 黑名单升级为“正则+关键词”双引擎，并增加对 Telegram 等 App 的手动豁免
// [稳定] 彻底移除不稳定的 V2Ray 内部加载器，改用原生 Protobuf 解析
// [状态] 完整无省略版, 生产级可用
...
func loadGeodata() {
	...
	if err := proto.Unmarshal(rawBytes, &geositeList); err != nil {
		log.Printf("[Core] [依赖检查] 错误: 解析 geosite.dat 失败! 错误: %v", err)
		return
	}
	geodataMutex.Lock()
	...
	for _, line := range lines {
		...
		if len(parts) >= 2 {
			...
			if strings.HasPrefix(keyword, "regexp:") {
				ruleType, ruleValue = MatchTypeRegex, strings.TrimPrefix(keyword, "regexp:"); compiledRegex = regexp.MustCompile(ruleValue)
			...
			for _, pNode := range pool { if pNode.Domain == nodeStr { foundNode, aliasFound = pNode, true; break } }
			if !aliasFound { foundNode = parseNode(nodeStr) }
			if keyword != "" && foundNode.Domain != "" {
				routingMap = append(routingMap, Rule{ Type: ruleType, Value: ruleValue, CompiledRegex: compiledRegex, Node: foundNode, Strategy: strategy })
			}
		}
	}
}
...
func StartInstance(configContent []byte) (net.Listener, error) {
	...
	log.Println("[Core] [依赖检查] 正在检查系统依赖..."); if checkFileDependency("xray.exe") { log.Println("[Core] [依赖检查] ✅ xray.exe 匹配成功! [智能分流] 模式可用。") } else { log.Println("[Core] [依赖检查] ⚠️ xray.exe 未找到! 如需使用 [智能分流] 模式，请补充此文件。") }; if checkFileDependency("geosite.dat") { log.Println("[Core] [依赖检查] ✅ geosite.dat 匹配成功! 已准备好被 [智能分流] 模式调用。"); loadGeodata() } else { log.Println("[Core] [依赖检查] ⚠️ geosite.dat 未找到! [智能分流] 的域名规则可能无法生效。") }; if checkFileDependency("geoip.dat") { log.Println("[Core] [依赖检查] ✅ geoip.dat 匹配成功! 已准备好被 [智能分流] 模式调用。") } else { log.Println("[Core] [依赖检查] ⚠️ geoip.dat 未找到! [智能分流] 的 IP 规则可能无法生效。") }; log.Println("[Core] [系统提示] Xlink 内核已就绪。所有复杂路由策略 (Geosite/GeoIP) 将交由 xray.exe 统一处理。"); log.Println("------------------------------------")
	...
	log.Printf("[Core] Xlink Observer Engine (v20.3.1) Listening on %s [%s]", inbound.Listen, mode)
...
func handleGeneralConnection(conn net.Conn, inboundTag string) {
	...
	if err != nil { return }; wsConn, err := connectNanoTunnel(target, "proxy", firstFrame); if err != nil { return }
...
func connectNanoTunnel(target, outboundTag string, payload []byte) (*websocket.Conn, error) {
	...
	parts := strings.SplitN(settings.Token, "|", 2); secretKey := parts[0]; fallback := ""; if len(parts) > 1 { fallback = parts[1] }; socks5 := ""; if settings.ForwarderSettings != nil { socks5 = settings.ForwarderSettings.Socks5Address }
	tryConnectOnce := func() (*websocket.Conn, error) {
		...
		for _, rule := range routingMap {
			if match(rule, target) { targetNode = rule.Node; finalStrategy = rule.Strategy; if finalStrategy == "" { finalStrategy = settings.Strategy }; logMsg = fmt.Sprintf("[Core] Rule Hit -> %s | SNI: %s (Rule: %s, Algo: %s)", target, targetNode.Domain, rule.Value, finalStrategy); ruleHit = true; break }
		}
		if !ruleHit {
			...
			} else { return nil, errors.New("no nodes configured in pool") }
		}
		...
		if err != nil { return nil, err }; if backend.IP != "" { log.Printf("[Core] Tunnel -> %s (SNI) >>> %s:%s (Real) | Latency: %dms", targetNode.Domain, backend.IP, backend.Port, latency) }
		err = sendNanoHeaderV2(wsConn, target, payload, socks5, fallback); if err != nil { wsConn.Close(); return nil, err }; return wsConn, nil
	}
	conn, err := tryConnectOnce(); if err != nil && len(settings.NodePool) > 1 { log.Printf("[Core] Connect failed: %v. Retry...", err); return tryConnectOnce() }; return conn, err
}
