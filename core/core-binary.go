// core/core-binary.go (v9.0 - PhantomFlow Kernel)
// 适配服务端 v9.2/v9.3 的噪声算法
...
func StartInstance(configContent []byte) (net.Listener, error) {
	...
	log.Printf("[Core] Xlink PhantomFlow Engine (v9.0) Listening on %s", inbound.Listen)
	go func() {
		for {
...
func sendPhantomHandshake(wsConn *websocket.Conn, target string, secretKey string, payload []byte, socks5 string, fallback string) error {
	...
	// [修复点] 匹配服务端 v9.0+ 的线性同余算法
	// NoiseLen = (Magic * 17 + 13) % 71 + 20
	// 范围: 20 ~ 90 字节
